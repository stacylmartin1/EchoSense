...
enum class TaskType(val label: String, val id: String) {
  ...
  ECHOSENSE(label = "EchoSense Visual Assistance", id = "echosense"),
...
val TASK_ECHOSENSE =
  Task(
    type = TaskType.ECHOSENSE,
    icon = Icons.Outlined.Widgets,
    models = mutableListOf(),
    description = "On-device accessibility companion using LLM for visual scene description",
    docUrl = "https://ai.google.dev/edge/mediapipe/solutions/genai/llm_inference/android",
    sourceCodeUrl =
      "https://github.com/google-ai-edge/gallery/blob/main/Android/src/app/src/main/java/com/google/ai/edge/gallery/ui/echosense/EchoSenseViewModel.kt",
  )

/** All tasks. */
val TASKS: List<Task> =
  listOf(TASK_ECHOSENSE, TASK_LLM_ASK_IMAGE, TASK_LLM_ASK_AUDIO, TASK_LLM_PROMPT_LAB, TASK_LLM_CHAT)
